...
substrate.parameters = function(DS="bio.substrate", p=NULL, resolution="canada.east.highres") {
  ...
  if (DS=="conker") {
    ...
    p$conker_global_modelengine = "gam"  # if ==NULL, this means skip global model
    p$conker_global_modelformula = formula(
      substrate ~ s(plon,k=3, bs="ts") + s(plat, k=3, bs="ts") + s(plon, plat, k=100, bs="ts") +
    ...
    p$conker_phi = p$pres/5  # FFT-based methods when operating globally
    p$conker_nu = 0.5
